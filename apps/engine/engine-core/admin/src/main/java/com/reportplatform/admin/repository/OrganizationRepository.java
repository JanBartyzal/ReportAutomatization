...
public interface OrganizationRepository extends JpaRepository<OrganizationEntity, UUID> {
    ...
    @Query("SELECT o FROM OrganizationEntity o WHERE o.parent IS NULL AND o.active = true")
    List<OrganizationEntity> findTopLevelOrganizations();

    @Query("SELECT o FROM OrganizationEntity o WHERE o.type = :type AND o.active = true")
    List<OrganizationEntity> findByType(OrganizationEntity.OrganizationType type);

    boolean existsByCode(String code);

    @Query("SELECT COUNT(o) > 0 FROM OrganizationEntity o WHERE o.parent.id = :parentId")
