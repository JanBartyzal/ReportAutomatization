...
Only tables meeting the confidence threshold (default 0.85) are returned.
"""
...
from src.config import METATABLE_CONFIDENCE_THRESHOLD
from src.service.pptx_parser import TableDataParsed, TableRowData, TextBlockData
...
class MetaTableDetector:
    ...
    def __init__(self, confidence_threshold: float | None = None) -> None:
        self._threshold = (
            ...
            else METATABLE_CONFIDENCE_THRESHOLD
        )
    ...
    def _try_detect(self, text: str) -> _CandidateTable | None:
        ...
        Args:
            text: Multi-line text from a shape.

        Returns:
            The highest-confidence ``_CandidateTable``, or None.
        """
    ...
    def _try_delimiter(lines: list[str], delimiter: str, label: str) -> _CandidateTable | None:
        ...
        Args:
            lines: Non-empty lines of text.
            delimiter: Delimiter string.
            label: Human-readable label for logging.

        Returns:
            A ``_CandidateTable`` if the structure looks tabular, else None.
        """
        ...
        # Require majority of lines to have the same column count
        ...
        # Filter to only lines matching the expected column count
        valid_lines = [
    ...
    def _try_pipe(lines: list[str]) -> _CandidateTable | None:
        ...
        # Pipe tables are typically quite reliable
    ...
    def _try_multispace(lines: list[str]) -> _CandidateTable | None:
        ...
        # Space-based detection is less reliable, so penalise slightly
...
def _compute_confidence(consistency: float, col_count: int, row_count: int) -> float:
    ...
    Factors:
    - Column-count consistency across rows (higher is better)
    - Number of columns (more columns = more likely a real table)
    - Number of rows (more rows = more likely a real table)

    Args:
        consistency: Fraction of rows with the most common column count.
        col_count: Number of columns.
        row_count: Number of data rows (including header).
    ...
    # Column bonus (2 cols = 0.1, 3+ cols = 0.2)
    if col_count >= 3:
        score += 0.2
    elif col_count >= 2:
        score += 0.1

    # Row bonus (more rows = more confident)
    if row_count >= 5:
        score += 0.2
    elif row_count >= 3:
        score += 0.15
