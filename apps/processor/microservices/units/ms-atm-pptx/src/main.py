"""MS-ATM-PPTX entry point.

Starts the async gRPC server on the configured port, registers the
``PptxAtomizerService`` servicer and health check, and waits for
termination signals.
"""
...
from atomizer.v1 import pptx_pb2_grpc  # type: ignore[import-untyped]

from src.config import GRPC_PORT, SERVICE_NAME
from src.service.pptx_service import PptxAtomizerService
...
async def _serve() -> None:
    ...
    # Register the PPTX atomizer servicer
    servicer = PptxAtomizerService()
    pptx_pb2_grpc.add_PptxAtomizerServiceServicer_to_server(servicer, grpc_server.server)
