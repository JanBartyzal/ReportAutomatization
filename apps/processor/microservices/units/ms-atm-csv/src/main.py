"""MS-ATM-CSV entry point.

Starts the async gRPC server on the configured port, registers the
``CsvAtomizerService`` servicer and health check, and waits for
termination signals.
"""
...
from atomizer.v1 import csv_pb2_grpc  # type: ignore[import-untyped]

from src.config import GRPC_PORT, SERVICE_NAME
from src.service.csv_service import CsvAtomizerService
...
async def _serve() -> None:
    ...
    # Register the CSV atomizer servicer
    servicer = CsvAtomizerService()
    csv_pb2_grpc.add_CsvAtomizerServiceServicer_to_server(servicer, grpc_server.server)
