...
def _create_merged_cells_workbook() -> bytes:
    ...
    # Merge A2:A3 (the "Group A" label spans two rows)
...
def _create_empty_rows_workbook() -> bytes:
    ...
    # Row 3 is empty (but openpyxl may still count it if row 4 has data)
...
class TestExtractStructure:
    ...
    def test_single_sheet_structure(self) -> None:
        ...
        assert sheet.row_count == 3  # 1 header + 2 data rows
    ...
    def test_multi_sheet_structure(self) -> None:
        ...
        assert structure.sheets[0].sheet_index == 0
        assert structure.sheets[1].sheet_index == 1
...
class TestExtractSheetContent:
    ...
    def test_merged_cells_are_filled(self) -> None:
        ...
        # After unmerge, both row 2 and row 3 should have "Group A" in column 0
        assert content.rows[0].cells[0] == "Group A"
        assert content.rows[1].cells[0] == "Group A"

    def test_empty_rows_skipped(self, monkeypatch: pytest.MonkeyPatch) -> None:
        """With EMPTY_ROW_THRESHOLD=0, rows with zero non-empty cells are skipped."""
        monkeypatch.setattr("src.service.excel_parser.EMPTY_ROW_THRESHOLD", 0)
        ...
        # Should have 2 data rows (Alice and Bob), empty row 3 skipped
