...
# Currency: $100, 100 CZK, 1 234,56 Kč, €100.00
...
class DataTypeDetector:
    ...
    Usage::
        ...
        col_type = detector.detect_column_type(["1 234,56", "5 678,90", "100"])
        # -> "NUMBER"
    """
    ...
    def detect_column_type(self, values: list[str]) -> str:
        ...
        The order of checks matters: PERCENTAGE is checked first (to avoid
        matching as NUMBER), then CURRENCY, then NUMBER, then DATE.

        Args:
            values: List of string cell values for a single column.

        Returns:
            One of STRING, NUMBER, DATE, CURRENCY, PERCENTAGE.
        """
        # Filter non-empty values and sample
        ...
        # Count matches for each type (order matters)
    ...
    def _is_percentage(value: str) -> bool:
        """Check if value matches a percentage pattern."""
        return bool(PERCENTAGE.match(value))

    @staticmethod
    def _is_currency(value: str) -> bool:
        """Check if value matches a currency pattern.

        Excludes plain numbers to avoid false positives.
        """
        if not CURRENCY.match(value):
            return False
        # Must have a currency symbol or suffix to distinguish from plain number
    ...
    def _is_number(value: str) -> bool:
        """Check if value matches a number pattern (standard or Czech)."""
        return bool(STANDARD_NUMBER.match(value) or CZECH_NUMBER.match(value))

    @staticmethod
    def _is_date(value: str) -> bool:
        """Check if value matches a common date pattern."""
