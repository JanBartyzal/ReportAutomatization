"""Shared Blob Storage client for downloading templates and uploading generated reports.
...
class GenPptxBlobClient:
    ...
    async def download_template(self, template_id: str, blob_url: str) -> bytes:
        ...
        container, blob_path = _parse_blob_url(blob_url, TEMPLATES_CONTAINER)
    ...
    async def upload_generated(self, report_id: str, pptx_bytes: bytes) -> str:
        ...
        logger.info("Uploading generated report report_id=%s (%d bytes)", report_id, len(pptx_bytes))
        return await self._client.upload_bytes(
            GENERATED_CONTAINER, blob_path, pptx_bytes, content_type=_PPTX_CONTENT_TYPE,
        )
...
class GenXlsBlobClient:
    """High-level blob client for Excel report generation.

    Usage::

        async with GenXlsBlobClient() as client:
            template_bytes = await client.download_template("tmpl-123", "templates/tmpl-123/v1/report.xlsx")
            url = await client.upload_generated("rpt-456", xlsx_bytes)
    """
    ...
    async def download_template(self, template_id: str, blob_url: str) -> bytes:
        """Download an Excel template as raw bytes."""
        container, blob_path = _parse_blob_url(blob_url, TEMPLATES_CONTAINER)
        logger.info("Downloading template template_id=%s from %s/%s", template_id, container, blob_path)
        return await self._client.download_bytes(container, blob_path)

    async def upload_generated(self, report_id: str, xlsx_bytes: bytes) -> str:
        """Upload a generated Excel report to blob storage."""
        blob_path = f"{report_id}/report.xlsx"
        logger.info("Uploading generated report report_id=%s (%d bytes)", report_id, len(xlsx_bytes))
        return await self._client.upload_bytes(
            GENERATED_CONTAINER, blob_path, xlsx_bytes, content_type=_XLSX_CONTENT_TYPE,
        )
    ...
    async def __aenter__(self) -> GenXlsBlobClient:
        return self
