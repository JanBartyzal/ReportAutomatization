...
**Microservices:** 25 service units (frontend … engine-core:batch)  
...
| 2 | **Edge Layer** | API Gateway (Traefik) jako jediný vstupní bod. SSL terminace, rate limiting, ForwardAuth pro validaci Azure Entra ID tokenů. | router, engine-core:auth |
| 3 | **Ingestion Layer** | Streaming příjem souborů, antivirová kontrola (ClamAV/ICAP), sanitizace (odstranění maker), uložení do Blob Storage, trigger N8N workflow. | engine-ingestor, engine-ingestor:scanner |
| 4 | **Orchestration Layer** | N8N workflow engine řídí celý processing pipeline: routing dle typu souboru, batch zpracování, retry/circuit breaker, Dead Letter Queue. | engine-orchestrator, engine-orchestrator:dlq |
| 5 | **Processing Layer (Atomizers)** | Bezstavové kontejnery pro extrakci dat z konkrétního formátu. Volány výhradně z N8N. Výsledky ukládají jako URL reference na Blob, ne jako inline payload. | processor-atomizers:pptx, -XLS, -PDF, -CSV, -AI, -CLN |
| 6 | **Persistence Layer (Sinks)** | Write-optimalizované API pro strukturovaná data (PostgreSQL), vector embeddings (pgVector), audit logy. CQRS: oddělený read model (engine-data:query, engine-data:dashboard). | engine-data:sink-tbl/DOC/LOG, engine-data:query, engine-data:dashboard |
...
- **API Gateway (Traefik):** Routing `/api/auth` → engine-core:auth, `/api/upload` → engine-ingestor, `/api/query` → engine-data:query. SSL terminace, rate limiting (429 Too Many Requests), ForwardAuth middleware.
...
- **Sanitizace:** Automatické odstranění VBA maker a externích odkazů z Office dokumentů (engine-ingestor:scanner) před předáním Atomizerům.
...
**Požadavky:**
- **Pipeline Workflow:** `Webhook (new_file)` → `Get Metadata` → `Router dle file type` → `Call Atomizer` → `Apply Schema Mapping (engine-data:template)` → `Store (engine-data (sink modules))`
...
- Embeddings generovány asynchronně přes processor-atomizers:ai po uložení dokumentu.
...
- Záznamy jsou read-only přístupné z engine-data:query a Admin UI.
...
- **engine-data:query:** CQRS read API. Materialized views pro nejčastější dotazy. Redis caching s TTL.
- **engine-data:dashboard:** Agregační endpointy pro grafy a souhrny. SQL over JSONB tabulky s GROUP BY / ORDER BY z UI konfigurace.engine-data:dashboard musí zobrazovat i data pocházející z formulářů, nejen z parsovaných souborů. Zdroj dat je transparentní (flag `source_type: FORM / FILE`)
- **engine-data:search:** Full-text search (PostgreSQL FTS nebo ElasticSearch). Vector search přes pgVector pro sémantické dotazy.
...
- **N8N integrace:** engine-data:template je voláno z N8N workflow **PŘED** zápisem do DB.
- **Excel import do formuláře** - nopé volání, už nejen z N8N pipeline. engine-data:template dostane nový endpoint `POST /map/excel-to-form`.
...
|---|---|---|---|---|---|---|
| 1 | **frontend** | Frontend SPA | React SPA – upload, viewer, dashboardy, notifikace (WebSocket/SSE), MSAL auth | FS09, FS11 | React 18 + Vite + TS + Tailwind | **XL** |
| 2 | **router** | API Gateway | Traefik – routing, SSL, rate limiting, ForwardAuth | FS01 | Traefik (config) | **S** |
| 3 | **engine-core:auth** | Auth Service | Validace Entra ID tokenů, RBAC engine, KeyVault integrace, API key validace | FS01, FS07 | Java 21 + Spring Boot | **L** |
| 4 | **engine-ingestor** | File Ingestor | Streaming upload, MIME validace, metadata zápis, sanitizace, trigger N8N | FS02 | Java 21 + Spring Boot | **L** |
| 5 | **engine-ingestor:scanner** | Security Scanner | Antivirová kontrola přes ICAP/ClamAV sidecar | FS02 | ClamAV (sidecar) | **S** |
| 6 | **engine-orchestrator** | N8N Orchestrator | Business workflow engine – routing, batch, retry, circuit breaker, DLQ | FS04 | N8N (JSON workflows) | **L** |
| 7 | **processor-atomizers:pptx** | PPTX Atomizer | Extrakce struktury, textů, tabulek a slide images z PPTX | FS03 | Python + FastAPI | **L** |
| 8 | **processor-atomizers:xls** | Excel Atomizer | Parsování Excel per-sheet do JSON, partial success handling | FS03, FS10 | Python + FastAPI | **M** |
| 9 | **processor-atomizers:pdf** | PDF/OCR Atomizer | OCR a extrakce textu ze skenovaných PDF | FS03 | Python + FastAPI | **M** |
| 10 | **processor-atomizers:csv** | CSV Atomizer | Konverze CSV na strukturovaný JSON | FS03 | Python + FastAPI | **S** |
| 11 | **processor-atomizers:ai** | AI Gateway | LiteLLM integrace, sémantická analýza, MetaTable logic, cost control | FS03, FS12 | Python + FastAPI | **L** |
| 12 | **processor-atomizers:cleanup** | Cleanup Worker | Cron pro mazání dočasných souborů z Blob po expiraci | FS03 | Python (CronJob) | **S** |
| 13 | **engine-data:sink-tbl** | Table API (Sink) | Ukládání strukturovaných dat (tabulky, OPEX) do PostgreSQL | FS05 | Java 21 + Spring Boot | **M** |
| 14 | **engine-data:sink-doc** | Document API (Sink) | Ukládání nestrukturovaného JSONu + vector embeddings (pgVector) | FS05 | Java 21 + Spring Boot | **M** |
| 15 | **engine-data:sink-log** | Log API (Sink) | Audit trail zpracování souborů – append-only processing logy | FS05 | Java 21 + Spring Boot | **S** |
| 16 | **engine-data:query** | Query API (Read) | CQRS read model, Redis caching, optimalizované čtení pro FE | FS06 | Java 21 + Spring Boot | **M** |
| 17 | **engine-data:dashboard** | Dashboard Aggregation | Grafy, souhrny, GROUP BY/SORT, SQL nad JSON tabulkami | FS06, FS11 | Java 21 + Spring Boot | **L** |
| 18 | **engine-data:search** | Search Service | Full-text search (PostgreSQL FTS / ES) + vector search | FS06 | Java 21 + Spring Boot | **M** |
| 19 | **engine-core:admin** | Admin Backend | Správa rolí, holdingová hierarchie, secrets, API keys, Failed Jobs UI | FS07, FS08 | Java 21 + Spring Boot | **L** |
| 20 | **engine-reporting:notification** | Notification Center | In-app (WS/SSE), e-mail (SMTP/SendGrid), granulární nastavení | FS13 | Java 21 + Spring Boot | **M** |
| 21 | **engine-data:template** | Template Registry | UI pro mapování sloupců, learning z historie, voláno z N8N | FS15 | Java 21 + Spring Boot | **L** |
| 22 | **engine-core:versioning** | Versioning Service | Verzování dat (v1→v2), diff tool pro zobrazení změn | FS14 | Java 21 + Spring Boot | **M** |
| 23 | **engine-core:audit** | Audit & Compliance | Immutable logy, read access log, AI audit, export | FS16 | Java 21 + Spring Boot | **M** |
| 24 | **processor-generators:mcp** | MCP Server (AI Agent) | AI agenti, On-Behalf-Of flow, token dědění, quotas | FS12 | Python + FastAPI | **L** |
| 25 | **engine-core:batch** | Batch & Org Service | Seskupování do batchů, holdingová metadata, RLS enforcement | FS08 | Java 21 + Spring Boot | **M** |
| 26 | **engine-reporting:lifecycle** | Report Lifecycle Service | FS17 | Java 21 + Spring Boot | **L** |
| 27 | **engine-reporting:pptx-template** | PPTX Template Manager | FS18 | Java 21 + Spring Boot | **L** |
| 28 | **processor-generators:pptx** | PPTX Generator | FS18 | Python + FastAPI | **L** |
| 29 | **engine-reporting:form** | Form Builder & Data Collection | FS19 | Java 21 + Spring Boot | **XL** |
| 30 | **engine-reporting:period** | Reporting Period Manager | FS20 | Java 21 + Spring Boot | **M** |
...
|---|---|---|---|---|
| **P1** | MVP Core | router, engine-core:auth, engine-ingestor, engine-ingestor:scanner, engine-orchestrator, processor-atomizers:pptx, engine-data:sink-tbl, engine-data:sink-log, frontend (základní) | FS01, FS02, FS03-PPTX, FS04, FS05, FS09-basic | Funkční upload + extrakce PPTX + základní viewer |
...
| **P3** | Intelligence & Admin | engine-core:admin, engine-core:batch, processor-atomizers:ai, processor-generators:mcp, engine-data:template | FS07, FS08, FS12, FS15 | Holdingová hierarchie + AI integrace + schema mapping |
| **P4** | Lifecycle + Period Mgmt  |engine-reporting:lifecycle, engine-reporting:period | | Řízení OPEX cyklu, deadliny, stavový automat |
| **P5** | Form Builder – centrální (engine-reporting:form, Excel export/import) | | | Sběr dat bez Excelu po e-mailu |
| **P6** | Enterprise Features | engine-reporting:notification, engine-core:versioning, engine-core:audit, engine-data:search, engine-data:dashboard (extended) | FS11, FS13, FS14, FS16 | Plná enterprise výbava: versioning, diff tool, compliance |
| **P7** | PPTX Generator (engine-reporting:pptx-template, processor-generators:pptx) | | |  Automatické generování standardizovaných reportů |
| **P8** | Advanced Period Mgmt (deadliny, eskalace, as-is srovnání) | | | Plná správa reportingových cyklů |
| **P9** | DevOps Maturity | Observability stack (Prometheus, Grafana, Loki, OTEL), CI/CD pipelines | FS99 | Production-ready: monitoring, tracing, automated pipelines |
| **P10** | FS22 Advanced Comparison (placeholder) | | |  Granulární srovnání period |
...
|---|---|---|---|---|---|
| R1 | N8N Payload Size: přenášení velkých JSONů (base64 obrázky) způsobí OOM crash | M | KRITICKÝ | Atomizery NIKDY neposílají binary inline. Vždy ukládají do Blob a vracejí URL. Max. payload limit: 5 MB na N8N webhook. | processor-atomizers, engine-orchestrator |
| R2 | Auth Token Propagation: desynchronizace tokenů mezi FE → GW → N8N → Backend | M | VYSOKÝ | Strict Token Propagation policy. Dapr middleware automaticky propaguje `Authorization` header. E2E testy pokrývají celý auth flow. | router, engine-core:auth |
...
| R4 | Komplexita N8N workflows: JSON workflow soubory se stanou neudržitelnými | M | STŘEDNÍ | Workflows verzovány v Gitu jako JSON. Code review povinný. Standardizované naming konvence pro všechny workflow uzly. | engine-orchestrator |
...
| engine-orchestrator | engine-ingestor (webhook), processor-atomizers, engine-data (sink modules) | HARD – orchestrátor bez zdrojů a sinks nefunguje |
| engine-data:sink-tbl/DOC | PostgreSQL (RLS, Flyway migrace) | HARD – DB musí být migrována před nasazením |
| engine-data:template | engine-orchestrator (volání), PostgreSQL (mapování history) | SOFT – N8N může fungovat bez TMPL (bez normalizace) |
...
|---|---|---|---|
| router | engine-core:auth | REST (ForwardAuth) | Sync |
| router | engine-ingestor | REST | Sync |
| router | engine-data:query | REST | Sync |
| engine-ingestor | engine-ingestor:scanner | ICAP / gRPC | Sync |
| engine-ingestor | engine-orchestrator | REST (Webhook) | Async (fire & forget) |
| engine-orchestrator | processor-atomizers | REST | Sync (within workflow) |
| engine-orchestrator | engine-data (sink modules) | REST | Sync |
| engine-orchestrator | engine-data:template | REST | Sync |
| engine-orchestrator | engine-reporting:notification | REST / Pub-Sub | Async |
| engine-reporting:notification | frontend | WebSocket / SSE | Push |
| engine-data:query | Redis | TCP | Cache lookup |
| engine-data (sink modules) | PostgreSQL | TCP | Write |
| engine-core:audit | PostgreSQL | TCP | Append-only write |
...
| **ForwardAuth** | Traefik middleware přesměrovávající každý příchozí request na engine-core:auth pro validaci tokenu PŘED předáním backend službě. |
